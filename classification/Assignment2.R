...
# Try rpart without cp
library(rpart)
carTree = rpart(shouldBuy~.,data=train,method="class",control=rpart.control(minsplit=10))
carTree
...
carTree = rpart(shouldBuy~.,data=train,method="class",control=rpart.control(minsplit=10, cp=0.005))
carTree
predCar=predict(carTree,newdata=test[,-7],type="class")
treeCM=table(test[,7],predCar)
treeCM
